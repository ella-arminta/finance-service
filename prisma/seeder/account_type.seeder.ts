...
async function main() {
  ...
  const account_types = await prisma.account_Types.createMany({
    data: [
      { name: 'Asset', description: "Asset : Kas, Bank, Giro, dll" ,created_at: new Date(), updated_at: new Date() },
      ...
      { name: 'Pendapatan', description: "Pendapatan", created_at: new Date(), updated_at: new Date() },
